    ...
    if ($file['error'] == UPLOAD_ERR_OK) {
        ...
        if (strtolower(pathinfo($file['name'], PATHINFO_EXTENSION)) == 'csv') {
            ...
            try {
                ...
                $stmt = $conn->prepare("INSERT INTO " . TABLE_WHITELIST . " (email, nama_pegawai, role, gbpekema_id) VALUES (?, ?, ?, ?) 
                                        ON DUPLICATE KEY UPDATE nama_pegawai = VALUES(nama_pegawai), role = VALUES(role), gbpekema_id = VALUES(gbpekema_id)");
                $added_count = 0;
                $updated_count = 0;
                while (($data = fgetcsv($handle)) !== FALSE) {
                    if (count($data) >= 3) {
                        ...
                        $gbpekema_id = isset($data[3]) ? (int)trim($data[3]) : null;
                        $valid_roles = ['admin', 'user', 'supervisor', 'senior_officer', 'licensee'];
                        if (filter_var($email, FILTER_VALIDATE_EMAIL) && !empty($nama_pegawai) && in_array($role, $valid_roles)) {
                            $stmt->bind_param("sssi", $email, $nama_pegawai, $role, $gbpekema_id);
                            $stmt->execute();
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['update_user'])) {
    ...
    if (!empty($email) && !empty($nama_pegawai) && !empty($role) && $id > 0) {
        $stmt = $conn->prepare("UPDATE " . TABLE_WHITELIST . " SET email = ?, nama_pegawai = ?, role = ?, gbpekema_id = ? WHERE id = ?");
        $stmt->bind_param("sssii", $email, $nama_pegawai, $role, $gbpekema_id, $id);
        if ($stmt->execute()) {
...
function force_utf8($data)
{
    ...
    return ($data === null) ? $data : mb_convert_encoding($data, 'UTF-8', 'UTF-8');
}

// Dapatkan senarai semua pengguna untuk dipaparkan
$users = [];
$sql = "SELECT au.id, au.email, au.nama_pegawai, au.role, au.last_login, au.gbpekema_id, g.nama as company_nama 
        FROM " . TABLE_WHITELIST . " au
        LEFT JOIN gbpekema g ON au.gbpekema_id = g.id
        ORDER BY au.nama_pegawai ASC";
...
$companies = [];
$c_res = $conn->query("SELECT id, nama FROM gbpekema ORDER BY nama ASC");
if($c_res) {
...
<head>
    ...
    <script>
        ...
        function openEdit(userId) {
            ...
            window.dispatchEvent(new CustomEvent('open-edit', { detail: user }));
        }
    </script>
</head>
...
<body class="bg-gray-100">
    ...
    <main class="container mx-auto p-4 sm:p-6 lg:p-8" x-data="{ editModalOpen: false, userToEdit: {} }"
            ...
            <div class="bg-white p-6 rounded-xl shadow-lg border border-gray-200">
                ...
                <form action="manage_users.php" method="POST" enctype="multipart/form-data" class="space-y-4">
                    <p class="text-sm text-gray-600">
                        <a href="template_pengguna.csv" class="text-blue-600 hover:underline font-medium">Muat Turun
                            Templat Di Sini</a>.
                    </p>
                    <div>
            ...
            <div class="bg-white p-6 rounded-xl shadow-lg border border-gray-200">
                ...
                <form action="manage_users.php" method="POST" class="space-y-4">
                    ...
                    <div>
                        ...
                        <input type="email" name="email" id="add-email" required pattern=".+@customs\.gov\.my$"
                            placeholder="pengguna@customs.gov.my"
                            class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500">
                    </div>
                    <div>
                        ...
                        <select name="role" id="add-role"
                            class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500">
                            <option value="user" selected>User (Capture Data)</option>
                            <option value="licensee">Licensee (Company)</option>
                            <option value="supervisor">Supervisor (View Only)</option>
                            <option value="senior_officer">Senior Officer (Approval)</option>
                            <option value="admin">Admin (Full Access)</option>
                        </select>
                    </div>
                    <div>
                        <label for="add-gbpekema_id" class="block text-sm font-medium text-gray-700">Syarikat (Untuk Licensee)</label>
                        <select name="gbpekema_id" id="add-gbpekema_id"
                            class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500">
                            <option value="">-- Pilih Syarikat (Jika Berkaitan) --</option>
        ...
        <div x-show="editModalOpen" style="display: none;" class="fixed z-50 inset-0 overflow-y-auto"
            ...
            <div class="flex items-center justify-center min-h-screen">
                <div class="fixed inset-0 bg-gray-500 bg-opacity-75 transition-opacity" @click="editModalOpen = false">
                </div>
                <div class="bg-white rounded-lg overflow-hidden shadow-xl transform transition-all sm:max-w-lg sm:w-full"
                    ...
                    <form action="manage_users.php" method="POST" @keydown.enter.prevent="">
                        <input type="hidden" name="id" x-model="userToEdit.id">
                        <div class="px-4 pt-5 pb-4 sm:p-6" @click.stop="">
                            <h3 class="text-lg leading-6 font-medium text-gray-900">Kemas Kini Pengguna</h3>
                            <div class="mt-4 space-y-4">
                                <div>
                                    <label for="edit-nama_pegawai" class="block text-sm font-medium text-gray-700">Nama
                                        Pegawai</label>
                                ...
                                <div>
                                    <label for="edit-role" class="block text-sm font-medium text-gray-700">Peranan
                                        (Role)</label>
                                    <select name="role" id="edit-role" x-model="userToEdit.role"
                                        class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500">
                                        <option value="user">User (Capture Data)</option>
                                        <option value="licensee">Licensee (Company)</option>
                                        <option value="supervisor">Supervisor (View Only)</option>
                                        <option value="senior_officer">Senior Officer (Approval)</option>
                                        <option value="admin">Admin (Full Access)</option>
                                ...
                                <div>
                                    <label for="edit-gbpekema_id" class="block text-sm font-medium text-gray-700">Syarikat</label>
                                    <select name="gbpekema_id" id="edit-gbpekema_id" x-model="userToEdit.gbpekema_id"
                                        class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500">
